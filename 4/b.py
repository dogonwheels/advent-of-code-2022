...
def is_overlapping(p1, p2):
    first_in_second = is_in_range(p1[0], p2) or is_in_range(p1[1], p2)
    second_in_first = is_in_range(p2[0], p1) or is_in_range(p2[1], p1)
    return first_in_second or second_in_first


sections = [parse_section(section) for section in (open("4/input").readlines())]

print(len([section for section in sections if is_overlapping(section[0], section[1])]))
