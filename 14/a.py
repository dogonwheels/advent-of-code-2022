...
the_void = max([pos[1] for pos in terrain.keys() if terrain[pos] == ROCK])
entered_the_void = False

while spawn not in terrain and not entered_the_void:
    position = spawn
    terrain[position] = SAND
    new_position = fall(terrain, position)

    while (new_position is not position) and not entered_the_void:
        terrain.pop(position)
        position = new_position
        terrain[position] = SAND
        new_position = fall(terrain, position)

        # print_terrain(terrain, (494, 0), (503, 9))
        entered_the_void = new_position[1] == the_void

print(len([pos for (pos, substance) in terrain.items() if substance is SAND]) - 1)
